...
public extension NMEAParserManager {
    ...
    func process(sentence: String, verbose: Bool = false) {
        ...
        let sentences: [String] = rawSentences.compactMap { part in
        ...
        DispatchQueue.concurrentPerform(iterations: sentences.count) { [weak self] index in
            ...
            if self.isSupportedGGASentence(sentence) {
                self.processGGASentence(sentence, verbose: verbose)
            } else if self.isSupportedRMCSentence(sentence) {
                self.processRMCSentence(sentence, verbose: verbose)
            } else if verbose {
